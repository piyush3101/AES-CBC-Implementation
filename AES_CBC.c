...
void AES_Decrypt(unsigned char* message, unsigned char* expandedKey, unsigned char* iv)
{
  unsigned char state[16];
...
int main()
{
    ...
    unsigned char iv[16]={0xda, 0x63, 0xbb, 0x53, 0x19, 0x65, 0xd1, 0xad,
                        0x7f, 0xef, 0xeb, 0x0c, 0x3b, 0x36, 0xfb, 0x85};
	...
	for(int i=0;i<lenOfPaddedMessage;i+=16)
	{
	    if(i>0)
	    {
	        for(int j=0;j<16;j++)
            {
                paddedMessage[i+j]^=paddedMessage[i+j-16];
            }
	...
	for(int i=0;i<lenOfPaddedMessage;i+=16)
    {
        for(int k=0;k<16;k++)
        {
            iv_2[k]=cipherstring[i+k];
        }
        AES_Decrypt(cipherstring + i,expandedKey,iv);
        for(int j=0;j<16;j++)
        {
            iv[j]=iv_2[j];
        }
